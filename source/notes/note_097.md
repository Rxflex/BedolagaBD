...
- **[id=657615|Shaban R.|08.06](https://t.me/c/2941121338/657615)** Пример неисправного inbound (grpc reality dest=www.microsoft.com) — контекст: после зачистки с внешними SNI начали падать; переход на селфстил — рабочий ответ:
  ```json
...
- **[id=658318|—|09.06](https://t.me/c/2941121338/658318)** Hysteria2 inbound с masquerade (маска под cloudflare):
  ...
  ```
- **[id=656368/656462|Антон К.|08.06](https://t.me/c/2941121338/656368)** Вопрос по hysteria inbound в логах ноды и порту 443 vs любой UDP — hysteria работает только с сертами (выпустить letsencrypt на домен), UDP порт любой.
- **[id=657759|suharik|08.06](https://t.me/c/2941121338/657759)** «какой гайд по хистерии?» — ссылку не дали, в чате разводили на 200$ (Леш Л.).
...
- **[id=653971/653977/654001|—/Руслан И.|08.06](https://t.me/c/2941121338/653971)** Серверный routing с leastPing балансером (RUMSK/RUNSK inbounds → balancer → fallback DIRECT):
  ```json
  "routing": {"rules":[
...
- **[id=654086/654050|Руслан И.|08.06](https://t.me/c/2941121338/654086)** Рекомендация: на клиенте в директ заводить не весь geosite:ru, а только те сервисы, которые реально не пускают с VPN (банки/озон/вк). Полные внешние роутинги типа RoscomVPN несовместимы/каша с серверным geosite. Клиентские конфиги не переносят жирные геофайлы — память уходит.
- **[id=653987|Руслан И.|08.06](https://t.me/c/2941121338/653987)** Сервисы «стучатся» из-за TUN-интерфейсов: приложения видят VPN. ВТБ палит VPN через `back.vtb.fp.f6.security` (антифрод-домен); озон/вк/макс при этом работают.
- **[id=654002/654001|Руслан И.|08.06](https://t.me/c/2941121338/654002)** Роутинг для ПК-клиента с запретом VPN-детекта — «Роутинг ВСЕ» — составлен в значительной мере из доменов этого чата.
- **[id=654111|Руслан И.|08.06](https://t.me/c/2941121338/654111)** Дока ремны: https://docs.rw/docs/learn/node-plugins (плагины для торрентов и пр.).
- **[id=655894/656803|Руслан И.|08.06](https://t.me/c/2941121338/655894)** Команда для проверки, куда уходит трафик на ноде (для отладки роутинга):
  ```bash
  docker exec -it remnanode tail -f /var/log/supervisor/xray.out.log | grep -- "->"
  ```
  Обязательно включить logging в json.
- **[id=656856/656853|Руслан И.|08.06](https://t.me/c/2941121338/656856)** Если правило не сработало — в inbound добавить:
  ```json
...
- **[id=660372/660382|Дмитрий Ш.|09.06](https://t.me/c/2941121338/660372)** Lava добавила домен `pay.lava.ru` (запрос в тех.поддержку), плаc рекуррентные платежи в приоритете, потом иностранки и карты.
...
- **[id=655191/655109|kurskyy/Фёдор С.|08.06](https://t.me/c/2941121338/655191)** Не применяются цены в периодах подписки → закомментировать прайс в `.env`, пересобрать бота, менять цены уже в кабинете/боте:
  ```bash
...
- **[id=654477|Андрей И.|08.06](https://t.me/c/2941121338/654477)** Ползунок «Автопродление» в админке — просьба, чтобы не менять значение в БД. Вручную выданные дни на суточный тариф все равно списывают плату (баг).
...
- **[id=657611|Chara F.|08.06](https://t.me/c/2941121338/657611)** Фича-запрос: конверсия тарифа без сохранения дней (бесплатная 0₽ может быть продлена на годы вперёд, конверсия в платный тариф всегда сохраняет дни).
...
- **[id=659893|Руслан И.|09.06](https://t.me/c/2941121338/659893)** Правильный nginx vhost для subscription-page:
  ```nginx
...
- **[id=659906|Руслан И.|09.06](https://t.me/c/2941121338/659906)** Ошибка `Reverse proxy and HTTPS are required` от subscription-page — healthcheck был на внешний адрес, надо на localhost.
...
- **[id=656868|Дмитрий Ш.|08.06](https://t.me/c/2941121338/656868)** Lava: у некоторых мерчантов не генерируется QR-код — антифрод режет генерацию, т.к. запросы с одного IP. Решение: домены `lava.ru` в клиентский роутинг (как сервисы Яндекса).
- **[id=656539/656481|eth|08.06](https://t.me/c/2941121338/656539)** lava.ru мульти IP проверка — все операторы полностью в блоке.
- **[id=660396|Дмитрий Ш.|09.06](https://t.me/c/2941121338/660098/660396)** Лава: сначала рекуррентные платежи, потом иностранки, потом карты.
- **[id=660572|Дмитрий Ш.|09.06](https://t.me/c/2941121338/660098/660572)** «Либо к нам на 3% по СБП и 6% по картам» (Platega).
...
- **[id=659192|Куvalda M.|09.06](https://t.me/c/2941121338/659192)** Оплата Lava: подключил ключи, в кабинете нет; оплата проходит, но баланс не начисляется.
...
- **[id=657782|Stas V.|08.06](https://t.me/c/2941121338/657782)** Ufw — «антисканер» ufw.
...
- **[id=656832|darkdragon|08.06](https://t.me/c/2941121338/656832)** «happ cryptolink ломает любой бот» (спорно — Фёдор С.: 2 месяца работало ок).
