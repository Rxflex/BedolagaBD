...
- Старое: не работает кнопка «Моя подписка» из-за webhook:close (id 224238, Name Lastname) — историческая бага с click.
- Бот: telegram webhook endpoints не настроены / «нет активных endpoints» — вебхук не создался, смотреть конфиг nginx (id 225354).
- В .env: убрать `web_api` в false → пропали ошибки на платежку/классик (id 225385, Vladimir Molotov).
...
## Тарифы в кабинете
- Переход между уровнями: тариф с уровнем 1 за 100р, уровень 2 за 150р → при переходе юзер доплатит 50р; на одном уровне разные цены → переход бесплатный (id 224272, Name Lastname).
...
- Автовыдача подписки в дефолтной группе не переключается при оплате, приоритет выше дефолта (id 226088, Vladimir Molotov).
- База тарифов и дефолт: «свободная» подписка «Свобода полная» из тарифа выдаётся юзерам (id 225357).
- Кабинет: вывод средств пока только в боте (id 224471, il migliore del mondo; id 224477, Name Lastname).
...
- AdGuardHome не перебивает рекламу ютуба: контент и реклама грузятся с одних серверов, рф-рекламу можно душить фильтрами (id 222255/222271, Valerii Bezkorovainyi / Áslan Chernov).
...
- v2raytun premium позволяет менять URL подписок (id 225313, Борис Бритва).
...
- На openwrt ставится SSClash (mihomo) для работы с подпиской (id 224838, Ramil Mers).
- Страница подписки берёт username из base64 внутри html, по uuid из ремны (id 222652/222654, —/Name Lastname).
...
- Платега: возврат средств — писать менеджеру, id транзакции или квитанция (id 225377, Arstan | Platega Sales). В кабинетах поле «секрет» и есть API от платеги (id 225303/225304).
...
- hip hosting: Варшава/Мск, 200 Мбит Швеция, -20% скидка (id 225963, Matvey Galkin), но жёстко режется трафик мск→варшава по SS туннелю (id 225782, Fuji San).
...
- Веб-кабинет может работать отдельно от бота (id 225409, Павел) — бот это бекенд; платежки в боте (id 225453/225479, Name Lastname).
...
- Отзыв рефералок: в карточке юзера в боте есть админка, добавить вручную реферала (id 224305/224228/224230, Zavulon/Name Lastname).
...
- GET /api/users/{userid} тянет username (id 222648, Name Lastname) — но фактически username в base64 в html саб-пейджа (id 222652).
